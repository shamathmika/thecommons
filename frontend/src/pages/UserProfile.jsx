...
export default function UserProfile() {
  ...
  // 2. Fetch all products to map Names for IDs
  useEffect(() => {
  ...
  const handleDeleteAccount = async () => {
    if (!window.confirm("ARE YOU SURE? This will permanently erase your profile, reviews, and village history. This cannot be undone!")) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        alert("Your data has been erased from the village. Goodbye!");
        logout();
        navigate('/');
      } else {
        alert("Failed to delete account. Please contact the village elder.");
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="user-profile-container">
      <div className="pixel-card profile-header-card">
        <h1 className="profile-title">{isOwnProfile ? "My Profile" : "User Profile"}</h1>
        <div className="profile-info">
          <p><strong>Name:</strong> {isOwnProfile ? user.name : "Citizen " + id}</p>
      ...
      <div className="profile-grid">
        <div className="pixel-card stats-card">
          ...
          <div className="user-reviews-list">
            {loadingReviews ? (
                  ...
                  <div key={rev.id} className="user-review-item">
                    <div className="user-review-meta">
                      ...
                      <div className="user-review-stars">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <PixelStar key={star} filled={star <= rev.rating} size={14} />
                        ))}
                      </div>
                    </div>
              ...
              <p className="no-data">No reviews written yet. Visit the shops to share your thoughts!</p>
            )}
          </div>
        </div>
        ...
        <div className="pixel-card actions-card">
          ...
          <div className="action-links">
             <Link to="/" className="pixel-btn">Browse Marketplace</Link>
             
             {isOwnProfile && (
               <button 
                 onClick={handleDeleteAccount} 
                 className="pixel-btn delete-btn"
                 style={{ marginTop: 'auto', backgroundColor: '#8b0000' }}
               >
